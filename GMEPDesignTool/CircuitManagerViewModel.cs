  ...
  public class CircuitManagerViewModel : ViewModelBase, IDropTarget
  {
    ...
    void IDropTarget.Drop(IDropInfo dropInfo)
    {
      ...
      if (sourceItem != null)
      {
        ...
        int targetIndex =
          targetItem != null ? targetCollection.IndexOf(targetItem) : targetCollection.Count;
    ...
    )
    {
      if (e.Action == NotifyCollectionChangedAction.Remove)
      {
        ...
        {
          var leftNotesToRemove = LeftNotes.Where(note => note.NoteId == removedItem.Id).ToList();
          foreach (var note in leftNotesToRemove)
          {
            LeftNotes.Remove(note);
          }

          var rightNotesToRemove = RightNotes.Where(note => note.NoteId == removedItem.Id).ToList();
