...
namespace GMEPDesignTool
{
  public class ElectricalPanel : ElectricalComponent
  {
    ...
    )
    {
      if (e.Action == NotifyCollectionChangedAction.Add)
      {
        foreach (ElectricalPanelNote newNote in e.NewItems)
        {
          ...
          newNote.Id = !String.IsNullOrEmpty(newNote.Id) ? newNote.Id : Guid.NewGuid().ToString();
    ...
    )
    {
      if (e.Action == NotifyCollectionChangedAction.Add)
      {
        foreach (ElectricalPanelNoteRel newNote in e.NewItems)
        {
          newNote.ProjectId = ProjectId;
          newNote.Id = !String.IsNullOrEmpty(newNote.Id) ? newNote.Id : Guid.NewGuid().ToString();
          newNote.NoteId = !String.IsNullOrEmpty(newNote.NoteId) ? newNote.NoteId : string.Empty;
          newNote.PanelId = id;
        }
      }
    }
    ...
    private async void CreateEquipFromPanelSchedule(ElectricalComponent comp, Circuit circ)
    {
      ...
      if (circ.Description.ToLower().EndsWith("va"))
      {
        ...
        if (vaStringArr.Length > 0)
        {
          ...
          if (equipVa > 0)
          {
            equipDescription = equipDescription.Replace(vaStringArr[vaStringArr.Length - 1], "");
          }
        }
      }
      if (circ.Description.ToLower().EndsWith("2p") || circ.Description.ToLower().EndsWith("3p"))
      {
        voltageId = 3;
        ...
        if (stringArr.Length > 0 && stringArr[stringArr.Length - 1].Contains("/"))
        {
          ...
          if (equipVa > 0)
          {
            equipDescription = equipDescription.Replace(stringArr[stringArr.Length - 1], "");
          }
        }
      }
      if (
    ...
    public void Circuit_PropertyChanged(object sender, PropertyChangedEventArgs e)
    {
      ...
      if (e.PropertyName == nameof(Circuit.Description) && sender is Circuit circuitDescription)
      {
        if (
        ...
        {
          comp = rightComponents.FirstOrDefault(c => c.CircuitNo == circuitDescription.Number);
          if (comp != null)
          {
            if (comp.Name == circuitDescription.Description)
            {
    ...
    public void SetKitchenDemandFactor()
    {
      ...
      else
      {
        ...
        while (i < notes.Count)
        {
          if (notes[i].Note.Contains("KITCHEN DEMAND") || String.IsNullOrEmpty(notes[i].Note))
          {
            notes.RemoveAt(i);
          }
          else
          ...
          for (int i = 0; i < component.Pole; i++)
          {
            ...
            switch (i)
            {
              case 0:
                ...
                if (component.GetType() == typeof(ElectricalEquipment) && KitchenDemandFactor < 1)
                {
