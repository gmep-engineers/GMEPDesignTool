...
namespace GMEPDesignTool
{
       ...
       public  float Va
        ...
        public void PopulateCircuits(string panelId, string projectId)
        {
            ...
            while (totalCircuits < NumBreakers)
            {
                ...
                {
                    Circuit newCircuit = new Circuit(Guid.NewGuid().ToString(), leftCircuits.Count * 2 + 1, 0, 0, "", 0, panelId, projectId, false);
                    leftCircuits.Add(newCircuit);
                    newCircuit.PropertyChanged += Circuit_PropertyChanged;
                }
                else
                {
                    Circuit newCircuit = new Circuit(Guid.NewGuid().ToString(), rightCircuits.Count * 2 + 2, 0, 0, "", 0, panelId, projectId, false);
        ...
        {
            if (e.PropertyName == nameof(Circuit.Custom) && sender is Circuit circuit)
            {
                if (!circuit.Custom)
                {
                    SetCircuitVa();
            ...
            foreach (var component in leftComponents)
            {
                ...
                if (circuitIndex != -1 && circuitIndex + component.Pole <= leftCircuits.Count)
                {
                    ...
                    for (int i = 0; i < component.Pole; i++)
                    {
                        ...
                        if (!leftCircuits[circuitIndex + i].Custom)
                        {
                            if (i == 0)
                            {
            ...
            foreach (var component in rightComponents)
            {
                ...
                if (circuitIndex != -1 && circuitIndex + component.Pole <= rightCircuits.Count)
                {
                    ...
                    for (int i = 0; i < component.Pole; i++)
                    {
                        ...
                        if (!rightCircuits[circuitIndex + i].Custom)
                        {
                            if (i == 0)
                            {
    ...
    public class Circuit : INotifyPropertyChanged
    {
        ...
        public Circuit(string _id, int _number, int _va, int _breakerSize, string _description, int _loadCategory, string _panelId, string _projectId, bool _custom)
        {
            id = _id;
            number = _number;
            va = _va;
